#include "synthizer/decoders/aac.hpp" // Assumendo questo percorso per l'header

#include "synthizer/byte_stream.hpp"    // Per LookaheadByteStream, ByteStream
#include "synthizer/config.hpp"         // Per SYNTHIZER_CONFIG_AAC_INIT_BUFFER_SIZE e potenzialmente CHANNELS_MAX
...
#include <vector>
#include <string>    // Per std::string, std::to_string
#include <cstring>   // Per std::memset, std::memcpy
#include <algorithm> // Per std::min

// Valore di default per il buffer di inizializzazione di AAC se non definito in config.hpp
// Dovrebbe essere sufficiente per FAAD2 per analizzare gli header iniziali.
// FAAD2_MIN_STREAMSIZE per AAC-LC è 768 byte. Qualche KB è più sicuro.
